...
const TicketPage = () => {
  ...
  useEffect(() => {
    ...
    clients
      ...
      .then(function (res) {
        setProgress(50);
        const initialOrders = Object.values(res.data.tickets);
        setOrders(initialOrders);
  ...
  return (
    <div>
      ...
      <div className=" ">
        ...
        {isAdmin ? 'Admin Ticket Request' : 'Ticket Request'}: ({totalOrder}) 
      ...
      <div className="dark:shadow-gray-400 mb-4 shadow-lg rounded-lg p-4">
        {orders.length === 0 ? (
          ...
          <div id="filteredProducts" className="flex flex-col space-y-4">
            {orders.map((order) => (
              <OrderProduct key={order.ticket_no} order={order.ticket} />
            ))}
          </div>
        )}
      </div>
