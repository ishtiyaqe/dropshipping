...
const OrderDetailsPopup = ({ data, onClose }) => {
    ...
    // // Update 'transactionImage' state when a file is selected
    // const handleTransactionImageChange = (event) => {
    //     const file = event.target.files[0];
    //     setTransactionImage(file);

    //     // Update the 'formData' object as well
    //     setFormData({ ...formData, t_img: file });
    // };
    const handleSubmit = async (e) => {
        e.preventDefault();
        const id = data.order.OrderP_id; // Assuming 'data' contains the order details
        ...
        clients.post(`OrderPaymentPayNowReq/${id}`, formData)
            .then(function (res) {
          ...
          <OrderPaySUccessFUllPage data={PaySuccessMessage}  />
            ...
            <div className="order-details-popup grid grid-cols-2 gap-2 h-fit  border rounded-md  shadow-md dark:shadow-gray-200 bg-white w-full p-4 text-black">
                <div className="popup-content">
                    ...
                    <div className='lg:flex lg:mt-4 mt-2 lg:space-x-6'>
                    <div>
                    <div className="text-xs">
                        <strong>Order ID:</strong> {data.order.OrderP_id}
                    </div>
                    <div className="text-xs">
                        <strong>Due:</strong> {data.Due.toFixed(2)}
                    </div>
                    <div className="text-xs">
                ...
                <div className="paynow  mt-4 p-2 text-center text-yellow-60 rounded-md shadow-md border text-white border-gray-200 theme_color theme_colorl  w-full">
                        Pay Now
                </div>
                    </button>
            </div>
