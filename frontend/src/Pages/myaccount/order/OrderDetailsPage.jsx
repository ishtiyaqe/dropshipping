        ...
        <div className="">
          <div className="mb-4 space-y-2 flex flex-col justify-center">
            ...
            {order.data.status === "Order Received" && (
              <button onClick={() => openPopup(order.data.OrderP_id)}>
            ...
            {order.data.status === "Partial Payment Received" && (
                 <button onClick={() => openPopup2(order.data.OrderP_id)}>
                 <div className="paynow p-2 text-center text-yellow-60 shadow-md text-white border-gray-200 rounded-md theme_color theme_colorl  w-full">
            ...
            <div className="flex lg:flex-row flex-col leading-5">
              <div>
                <div className="block lg:w-32 w-full lg:h-32 h-auto mb-4 rounded border border-gray-200 overflow-hidden">
                  <img src={order.data.image} alt={order.data.product.name} />
                </div>
              </div>
              <div className="lg:ml-3 ml-0 mb-4">
                <Link className=" text-2xl" to={`/product/${order.data.product.product_no}`} >
          ...
          <table className="table-auto grid grid-cols-2">
            ...
            <tbody>
              <tr className='flex flex-col'>
                ...
                <td className='text-xs border text-center border-gray-950 p-2 rounded-r-lg'><b>{order.data.price.toFixed(2)}</b></td>
                <td className='text-xs border text-center border-gray-950 p-2 rounded-r-lg'><b>{order.data.paid.toFixed(2)}</b></td>
                <td className='text-xs border text-center border-gray-950 p-2 rounded-r-lg'><b>{order.data.due.toFixed(2)}</b></td>
